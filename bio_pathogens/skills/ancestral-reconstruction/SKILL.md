...
description: >-
  Academic research toolkit for the Guyeux group (FEMTO-ST, University of Franche-Comte). Ancestral-state reconstruction on published-research MTBC phylogenies for peer-reviewed phylogeographic publications. Reconstruct ancestral geographic states on MTBC phylogenies.
  Parsimony (Fitch), ML marginal posteriors (MPPA), stochastic mapping,
  BEAST DTA XML generation, and iTOL migration arrow export.

  Use when: identifying when and how many times a lineage migrated between
  regions, comparing migration patterns to human movements, annotating
  phylogenies with ancestral locations, preparing BEAST discrete trait analyses.
