...
name: mtbc-deepen
description: >-
  Approfondissement systematique d'une etude MTBC en cours. Enchaine :
  bilan de l'etat du projet (/mtbc-bilan), revue de litterature elargie
  (/lit-review --wide, en s'appuyant en priorite sur tbmonitor-papers
  pour le sondage rapide du corpus TB pre-indexe ~190k papiers PubMed),
  inventaire des skills pertinents non encore mobilises, et synthese
  de nouvelles pistes d'approfondissement hierarchisees. Ou conclut
  honnetement qu'il n'y a plus rien a investiguer et que l'etude peut
  etre close.

  Use when: reprendre une etude apres une pause, chercher de nouvelles
  directions pour un projet qui stagne, decider si un projet merite
  encore du temps, preparer la suite apres une phase d'analyse terminee,
  faire le point complet avant une reunion.
