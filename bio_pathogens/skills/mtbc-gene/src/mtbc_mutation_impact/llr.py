...
def llr_of(gene: str, mutation: str, model=None, alphabet=None) -> dict:
    ...
    pos0 = pos - 1
    if pos0 >= len(prot):
        raise LLRError(f"pos {pos} beyond protein length {len(prot)} for {gene}")
    if prot[pos0] != ref:
        raise LLRError(f"WT mismatch for {gene} {mutation}: seq has {prot[pos0]} at {pos}")
    ...
    return {
        "gene": gene, "mutation": mutation, "ref": ref, "pos": pos, "alt": alt,
        "wt_len": len(prot), "p_wt": round(p_wt, 4), "p_alt": round(p_alt, 4),
        "llr": round(p_alt - p_wt, 4), "windowed": len(prot) > MAXLEN,
    }
...
def main(argv=None):
    ...
    if len(argv) != 2:
        print("usage: python -m mtbc_mutation_impact.llr <gene> <MUT>  e.g. eccE1 G346A",
              file=sys.stderr)
        return 2
