...
Vérifier avec `read_page` la présence d'un élément propre à l'état connecté (nom de
l'auteur, menu de compte, tableau de bord) avant de conclure qu'il faut se
connecter. Une page qui semble vide n'est pas une déconnexion : `get_page_text`
...
son rôle via `read_page`, ou son libellé.
