...
# deai-latex n'est pas toujours un skill frere : selon l'installation, c'est un skill
# personnel a cote de celui-ci, ou un skill du plugin `redaction` (un AUTRE plugin,
# donc hors de ${CLAUDE_PLUGIN_ROOT} qui ne resout que la racine de CE plugin-ci ;
# cf. plugins/CLAUDE.md § references entre plugins). Essayer plusieurs emplacements
# plausibles avant d'abandonner explicitement (bug vecu : le script echouait
# silencieusement des qu'un seul des candidats existait, sans dire lesquels avaient
# ete essayes).
_racine_plugin_courant = os.environ.get("CLAUDE_PLUGIN_ROOT")
_CANDIDATS_DEAI_LATEX = [_HERE.parent.parent / "deai-latex" / "scripts"]
if _racine_plugin_courant:
    _CANDIDATS_DEAI_LATEX.append(
        Path(_racine_plugin_courant).parent / "redaction" / "skills" / "deai-latex" / "scripts")
_CANDIDATS_DEAI_LATEX.append(
    Path.home() / "docs" / "environnement" / "plugins" / "redaction" / "skills" / "deai-latex" / "scripts")
for _cand in _CANDIDATS_DEAI_LATEX:
    if _cand.is_dir():
...
try:                                    # réutilisé, jamais réécrit : cf. /deai-latex
    ...
    tries = ", ".join(str(c) for c in _CANDIDATS_DEAI_LATEX)
    print(f"Scripts de /deai-latex introuvables ({exc}). "
          f"Ce script les reutilise plutot que de recompter les mots. "
          f"Emplacements essayes : {tries}",
          file=sys.stderr)
    raise SystemExit(3)
