...
# skill est absent de la machine (mp/mh) — cf. ~/.claude/skills/bdd/SKILL.md. `--root` est un
# parametre libre (bdd/actuelle ou bac a sable) : on ne journalise QUE s'il tombe sous la
# racine d'un store declare au registre (bdd_journal.store_for_path), transparent sinon.
...
def main():
    ...
    seen, written = set(), 0
    store_id = bdd_journal.store_for_path(str(root)) if bdd_journal is not None else None
    _ctx = (bdd_journal.ecriture(store_id, outil=__file__,
                                  motif=f"spdi_from_tblearn {len(todo)} accession(s) -> {root}",
                                  piste="AA3")
            if bdd_journal is not None and store_id is not None else nullcontext())
    with _ctx, open(manifest, "a", newline="") as mf:
