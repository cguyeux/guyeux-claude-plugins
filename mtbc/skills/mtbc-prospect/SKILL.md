...
5. Lire `~/.claude/knowledge/tuberculosis.md` (contexte MTBC transversal, pieges
   documentes, biais connus).
...
> `~/.claude/knowledge/tuberculosis.md`.
...
**Sauf `--dry-run`.** Inscrire les pistes retenues dans `pistes.md` **en
respectant strictement la convention `/pistes`** (cf.
`~/.claude/skills/pistes/SKILL.md`). **Mutation locale uniquement : on n'AJOUTE que
des lignes, jamais on ne reecrit un bloc, ne supprime, ni ne change l'etat ou le
texte d'une piste existante.** Deux modes de sortie, selon l'origine de la piste :
