...
3. Verifier si un cache partage `sra_geo_cache.csv` existe dans
   `~/.claude/cache/sra-geolocate/` ou dans le repertoire courant → le
...
- `--cache <path>` : chemin du cache persistant. Defaut :
  `~/.claude/cache/sra-geolocate/sra_geo_cache.csv`.
...
7. **Cache** : conserver les supp telecharges dans
   `~/.claude/cache/sra-geolocate/supp/$PMCID/` pour eviter les
