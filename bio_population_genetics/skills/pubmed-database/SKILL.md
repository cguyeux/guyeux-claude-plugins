...
### Parallel calls
- When running several ESearch / EFetch / ESummary queries for independent
  topics or ID batches, dispatch them in parallel (multiple tool calls per
  message). With an API key the limit is 10 req/sec, which comfortably
  absorbs 4-8 concurrent calls. Sequential loops over many independent
  queries are the dominant latency source in literature reviews.
- Keep sequential only the calls with genuine data dependencies
  (ESearch → EFetch using `webenv`/`query_key`).
