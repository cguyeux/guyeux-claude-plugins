...
- **Ouvrir une fois** : `tabs_context_mcp{createIfEmpty:true}` au début de la
  soumission crée UNE fenêtre avec son groupe. C'est la fenêtre de la soumission.
- **Réutiliser** : passer le `tabId` explicite à chaque `navigate`. Une soumission
  ...
  onglet. `tabs_create_mcp` ne sert qu'à préserver un état réellement parallèle, par
...
- **Fermer avant de rendre la main** : `tabs_close_mcp` sur chaque onglet en fin de
...
l'élément par son rôle ou son libellé via `read_page`.

**Une page qui semble vide après un enregistrement ne l'est pas.** `get_page_text`
n'expose que le texte statique, pas les valeurs des champs. Vérifier par
`read_page` ou par capture avant de tout ressaisir.
