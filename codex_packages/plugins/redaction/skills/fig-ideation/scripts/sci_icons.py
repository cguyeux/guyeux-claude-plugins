...
def phylopic_by_taxid(taxid: int) -> dict | None:
    ...
    node = (d.get("_links", {}).get("self", {}).get("href", "")
            .split("/nodes/")[-1].split("?")[0])
    return {"href": vec, "license": short_license(lic), "license_url": lic,
            "contributor": contrib, "taxid": taxid, "node": node,
            "build": d.get("build"),
            "title": (links.get("self") or {}).get("title", "")}


def phylopic_free_alternative(node: str, build: int | None) -> dict | None:
    """Chercher dans le MEME clade une silhouette non share-alike.

    L'image « primaire » d'un noeud est souvent en CC BY-SA alors que le clade en
    porte d'autres en CC0 : ecarter sans chercher prive d'une icone qui existe.
    """
    if not node or build is None:
        return None
    try:
        page = fetch_json(f"{PHYLOPIC}/images?build={build}"
                          f"&filter_clade={node}&page=0")
    except Exception:
        return None
    for it in (page.get("_links", {}).get("items") or []):
        href = it.get("href", "").split("?")[0]
        try:
            im = fetch_json(f"{PHYLOPIC}{href}?build={build}")
        except Exception:
            continue
        lk = im.get("_links", {})
        lic = (lk.get("license") or {}).get("href", "")
        if SHARE_ALIKE.search(lic):
            continue
        vec = (lk.get("vectorFile") or {}).get("href")
        if not vec:
            continue
        return {"href": vec, "license": short_license(lic), "license_url": lic,
                "contributor": (lk.get("contributor") or {}).get("title", "inconnu")}
    return None
...
def write_attribution(records: list[dict], path: Path) -> None:
    ...
    Le fichier est FUSIONNE, jamais ecrase : le flux naturel est un appel par icone,
    et un fichier ecrase apres huit appels serait incomplet en ayant l'air complet —
    exactement ce que ce fichier existe pour empecher.
    """
    existing: dict[str, str] = {}
    if path.is_file():
        for ln in path.read_text(encoding="utf-8").splitlines():
            m = re.match(r"%\s{3}(\S+\.pdf)\s:\s(.*)", ln)
            if m:
    ...
    for r in records:
        ...
        existing[r["file"]] = f"{src}, {ident}, {who}, {r['license']}"

    lines = ["% Attribution des icones — a reporter dans la legende ou les remerciements.",
             "% Genere par fig-ideation/scripts/sci_icons.py (fichier fusionne, non ecrase)."]
    lines += [f"%   {f} : {v}" for f, v in sorted(existing.items())]
    sa = [f for f, v in sorted(existing.items()) if SHARE_ALIKE.search(v)]
    if sa:
        lines.append("% ATTENTION licence share-alike (contamine la figure, donc l'article) :")
        lines += [f"%   {f} — verifier la politique de la revue." for f in sa]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"\nattribution fusionnee dans {path} ({len(existing)} icone(s))")
    ...
    if a.cmd == "taxon":
        for tx in a.taxids:
            ...
            if a.libre and SHARE_ALIKE.search(meta["license"]):
                alt = phylopic_free_alternative(meta.get("node", ""),
                                                meta.get("build"))
                if alt is None:
                    print(f"  taxid {tx} : ECARTE, {meta['license']} (share-alike), "
                          "aucune alternative libre dans le clade")
                    continue
                print(f"  taxid {tx} : primaire en {meta['license']}, "
                      f"remplacee par une {alt['license']} de {alt['contributor']}")
                meta.update(alt)
