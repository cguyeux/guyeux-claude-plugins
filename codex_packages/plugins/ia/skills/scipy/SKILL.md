...
# interp2d a ete RETIRE en SciPy 1.14.
# Grille reguliere -> RegularGridInterpolator (x, y strictement croissants) :
from scipy.interpolate import RegularGridInterpolator
interp = RegularGridInterpolator((x, y), Z.T, method='cubic')

# Evaluate at new points (produit cartesien)
x_new = np.linspace(0, 4, 50)
y_new = np.linspace(0, 4, 50)
XN, YN = np.meshgrid(x_new, y_new, indexing='ij')
Z_new = interp(np.stack([XN.ravel(), YN.ravel()], axis=-1)).reshape(XN.shape)

# Donnees eparses/irregulieres -> griddata :
#   from scipy.interpolate import griddata
#   Z_new = griddata((x_flat, y_flat), z_flat, (XN, YN), method='cubic')
...
# NB : integrate.romberg a ete RETIRE en SciPy 1.15 ; utiliser quad (adaptatif)
# ou, pour du Romberg-like sur callable, integrate.tanhsinh (>=1.15).
methods = ['quad', 'trapezoid', 'simpson']
times = []
