...
# observed_otus a ete RENOMME observed_features
observed = alpha.observed_features(counts)
...
```python
from skbio.diversity import beta_diversity   # beta.pw_distances a ete RETIRE
import numpy as np
...
# Bray-Curtis distance -> DistanceMatrix
bc_dm = beta_diversity("braycurtis", data, ids=ids)

# Weighted UniFrac : beta_diversity("weighted_unifrac", data, ids=ids,
...
def plot_microbiome_pcoa(abundance_df, metadata_df, group_col):
    ...
    dm = beta_diversity("braycurtis", abundance_df.values, ids=abundance_df.index)
...
When calculating diversity for many samples, pass the entire 2D array to `beta_diversity(...)` instead of looping through pairs.
