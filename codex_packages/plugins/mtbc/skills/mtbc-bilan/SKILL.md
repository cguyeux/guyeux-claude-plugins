...
description: >-
  ...
  artefacts de mémoire (cahier, état, pistes, AGENTS.md or CLAUDE.md fallback, JOURNAL). S'applique à
...
(AGENTS.md or CLAUDE.md fallback, cahier de labo, article, litterature, analyses, BDD), agrege les
...
2. Consulter `~/.Codex/knowledge/tuberculosis.md` s'il existe.
3. Lire `codes/mtbc/AGENTS.md or CLAUDE.md fallback` (conventions globales du depot ; l'inventaire des
...
3. Sanity-check : compter les presences parmi `AGENTS.md` or `AGENTS.md or CLAUDE.md fallback` fallback,
...
**Sources autorisees** : `AGENTS.md` or `AGENTS.md or CLAUDE.md fallback` fallback (integral) ; le cahier
...
legere) ; `~/.Codex/knowledge/tuberculosis.md` ; skill `mtbc-lineages`.
...
1. `Read AGENTS.md or CLAUDE.md fallback` integralement : titre, lignee, conventions, question
...
`Read AGENTS.md or CLAUDE.md fallback` limit 20 → titre + objectif. Identifier lignees
...
```bash
ls "../" "../../bio-population-genetics/skills/"
```
...
```bash
python3 recadrage_signals.py, once CCX-05 has ported recadrage <projet>
```
...
```bash
TEMPLATE=templates/bilan.latex
...
1. `ls /home/christophe/docs/codes/mtbc/`, puis filtrer : **inclure** les
   repertoires contenant `AGENTS.md` or `AGENTS.md or CLAUDE.md fallback` fallback ET (`article/` OU `analyses/` OU
