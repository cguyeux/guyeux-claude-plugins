...
SPDI_ANNOTATION_SCRIPT = Path(
    os.environ.get(
        "SPDI_ANNOTATION_PY",
        "~/docs/environnement/plugins/mtbc/skills/spdi-annotation/scripts/annotate_spdis.py",
    )
).expanduser()
H37RV_GFF3 = Path(
    os.environ.get(
        "MTBC_H37RV_GFF3",
        "~/docs/codes/mtbc/en_cours/investigate_phylo/resources/NC_000962.3.gff3",
    )
).expanduser()
H37RV_GB = Path(
    os.environ.get(
        "MTBC_H37RV_GB",
        "~/docs/codes/mtbc/en_cours/investigate_phylo/resources/NC_000962.3.gb",
    )
).expanduser()
