...
description: >-
  ...
  Portee : meta-skill de conduite de projet, pas de biologie — archivage tague,
  re-analyse claim par claim et blocage de la redaction s'appliquent a TOUT projet
  de recherche du depot, quel que soit l'organisme (MTBC, Yersinia, Leptospira).
...
1. Lire `AGENTS.md` or `AGENTS.md or CLAUDE.md fallback` fallback du projet (si present).
2. Lire `cahier_de_labo.md` ou `JOURNAL.md` (si present).
3. Lire `~/.Codex/knowledge/tuberculosis.md` pour le contexte transversal MTBC.
...
puis de reinitialiser la couche de connaissance (AGENTS.md or CLAUDE.md fallback, cahier, article)
tout en preservant les artefacts reutilisables (data, scripts, resultats).

### Etape 1 : Scan exhaustif du projet

1. **Lire AGENTS.md or CLAUDE.md fallback integralement** :
...
11. **Knowledge et memoires externes au projet** :
    - Lire `~/.Codex/knowledge/tuberculosis.md` et identifier les entrees
    ...
    - Lire `~/.codex/memories/ or project registers` et identifier les
...
- `cp AGENTS.md or CLAUDE.md fallback archives/.../CLAUDE_legacy.md`
...
1. Lire `~/.Codex/knowledge/tuberculosis.md`.
...
4. Retirer ces entrees de `~/.Codex/knowledge/tuberculosis.md` via Edit.
...
1. **AGENTS.md or CLAUDE.md fallback**, reecrire avec :
   - Mention du reboot et lien vers `archives/YYYY-MM-DD_reboot/`
   - Titre et lignee du projet (conserves depuis l'ancien AGENTS.md or CLAUDE.md fallback)
...
Projet reinitialise :
  AGENTS.md or CLAUDE.md fallback          : mis a jour (mention reboot)
...
3. Pour chaque repertoire restant :
   - `Read AGENTS.md or CLAUDE.md fallback` limit 50 → titre, lignee, statut.
...
Lire `~/.Codex/knowledge/tuberculosis.md` et extraire les sections
...
3. Pour chaque claim :
   - Assigner une **priorite P1-P4** (taxonomie de `/claim-check`,
     voir `../../bio-redac/skills/claim-check/references/CLAIM_TAXONOMY.md`).
...
5. **Reinsertion knowledge** (si claim confirme) :
   ...
   - Si oui : reinseree dans `~/.Codex/knowledge/tuberculosis.md` avec
