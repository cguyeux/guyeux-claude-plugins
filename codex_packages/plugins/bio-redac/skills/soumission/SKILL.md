...
Toute la soumission tient dans UNE fenêtre : `agent-browser --session soumission open <url>` au
début, puis la même session nommée utilisée avec `agent-browser open` pour enchaîner les pages
dans le même onglet, puis `agent-browser --session soumission close` avant de rendre la main, y compris quand le
...
1. Lire la mémoire du projet : `cahier_de_labo.md` (au moins les trois dernières
   entrées), `etat_des_decouvertes.md`, `pistes.md`, puis `AGENTS.md`, ou `CLAUDE.md` seulement comme registre historique.
...
Utiliser le skill `agent-browser` avec une session nommée propre au manuscrit. Suivre la boucle `open`, `snapshot -i`, action, nouveau `snapshot -i`, puis fermer la session avant de rendre la main. Activer les limites de domaine et les frontières de contenu quand elles sont disponibles. Ne jamais enregistrer un état d'authentification, saisir un secret ou franchir une confirmation légale. Si `agent-browser` ou un navigateur équivalent n'est pas disponible, rester en préparation locale et signaler le verrou au lieu de simuler l'action externe.
